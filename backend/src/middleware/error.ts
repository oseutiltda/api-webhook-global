...
export function errorHandler(err: any, _req: Request, res: Response, _next: NextFunction) {
  ...
  if (err instanceof SyntaxError && 'body' in err) {
    ...
    const bodyStr = typeof jsonError.body === 'string' 
      ? jsonError.body 
      : JSON.stringify(jsonError.body);
    ...
    if (err.message.includes('Unexpected token')) {
      ...
      if (match) {
        ...
        detalhesEspecificos = 'Valores de string devem estar entre aspas (ex: "S/N" ao invés de S/N). O sistema tentou corrigir automaticamente, mas ainda há problemas no JSON.';
      }
    }
    
    return res.status(jsonError.status || 400).json({ 
      error: 'JSON inválido',
      mensagem: 'O JSON enviado está mal formatado. O sistema tentou corrigir automaticamente, mas ainda há problemas. Verifique se todos os valores estão entre aspas.',
