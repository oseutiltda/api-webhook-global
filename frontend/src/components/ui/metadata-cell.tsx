...
export function MetadataCell({ metadata, className }: MetadataCellProps) {
  ...
  const getSummary = () => {
    ...
    if (parsedMetadata.codPessoa) parts.push(`Pessoa: ${parsedMetadata.codPessoa}`)
    if (parsedMetadata.cteId) parts.push(`CT-e: ${parsedMetadata.cteId}`)
    if (parsedMetadata.manifestId) parts.push(`CIOT: ${parsedMetadata.manifestId}`)
    if (parsedMetadata.nrciot) parts.push(`CIOT: ${parsedMetadata.nrciot}`)
    ...
    // Resumo
    if (parsedMetadata.resumo) {
      const { sucesso, falhas } = parsedMetadata.resumo
      if (sucesso !== undefined || falhas !== undefined) {
        parts.push(`${sucesso || 0} ok, ${falhas || 0} erro(s)`)
      }
    }
    
    // Etapa
    if (parsedMetadata.etapa) {
      parts.push(parsedMetadata.etapa)
    }

    return parts.length > 0 ? parts.join(' | ') : metadataString.substring(0, 50) + (metadataString.length > 50 ? '...' : '')
  }
