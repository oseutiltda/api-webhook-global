...
const CONTAS_RECEBER_BAIXA_BATCH_SIZE = Number(
  process.env.CONTAS_RECEBER_BAIXA_WORKER_BATCH_SIZE ?? '5',
);
...
/**
 * Verifica se o robô está em execução (CRFECH - Contas Receber Fechamento)
 */
const verificarExecucaoRobo = async (prisma: PrismaClient): Promise<number> => {
  try {
    const sql = `EXEC dbo.P_VERIFICAR_ROBO_INTEGRADOR_EXECUCAO_CRFECH`;
    ...
    logger.error({ error: error.message }, 'Erro ao verificar execução do robô CRFECH');
    return 0;
  }
};

/**
 * Altera status de execução do robô (CRFECH)
 */
const alterarExecucaoRobo = async (prisma: PrismaClient, roboExecutando: number): Promise<void> => {
  try {
    const sql = `
      EXEC dbo.P_ALTERAR_ROBO_INTEGRADOR_EXECUCAO_CR_FECH
        @RoboExecutando = ${roboExecutando};
    `;
    await prisma.$executeRawUnsafe(sql);
  } catch (error: any) {
    logger.error(
      { error: error.message, roboExecutando },
      'Erro ao alterar execução do robô CRFECH',
    );
    throw error;
  }
};
...
/**
 * Lista contas a receber não integradas (fechamento)
 */
const listarContasReceberFechamentoNaoIntegradas = async (
  prisma: PrismaClient,
): Promise<string[]> => {
  try {
    ...
    logger.error(
      { error: error.message },
      'Erro ao listar contas a receber fechamento não integradas',
    );
    return [];
  }
};
...
const lerDadosFatura = async (prisma: PrismaClient, document: string): Promise<Fatura | null> => {
  try {
    ...
    logger.error({ error: error.message, document }, 'Erro ao ler dados da fatura CR Baixa');
    return null;
  }
};
...
const lerDadosFilialFatura = async (
  ...
  try {
    ...
    logger.error({ error: error.message, document }, 'Erro ao ler dados da filial CR Baixa');
    return null;
  }
};
...
const lerDadosClienteFatura = async (
  ...
  try {
    ...
    logger.error({ error: error.message, document }, 'Erro ao ler dados do cliente CR Baixa');
    return null;
  }
};
...
const lerDadosContaContabilFatura = async (
  ...
  try {
    ...
    logger.error(
      ...
      'Erro ao ler dados da conta contábil CR Baixa',
    );
    return null;
  }
};
...
const lerDadosParcelaFatura = async (
  ...
  try {
    ...
    logger.error({ error: error.message, document }, 'Erro ao ler dados das parcelas CR Baixa');
    return [];
  }
};
...
const lerDadosItensFaturaFatura = async (
  ...
  try {
    ...
    logger.error(
      { error: error.message, document },
      'Erro ao ler dados dos itens da fatura CR Baixa',
    );
    return [];
  }
};
 ...
 */
const alterarContasReceberFechamentoParaProcessado = async (
  prisma: PrismaClient,
  document: string,
): Promise<void> => {
  try {
    ...
    `;
    await prisma.$executeRawUnsafe(sql);
    logger.info({ document }, 'Conta a receber fechamento marcada como processada');
  } catch (error: any) {
    logger.error(
      { error: error.message, document },
      'Erro ao marcar conta a receber fechamento como processada',
    );
    throw error;
  }
};
...
/**
 * Marca conta a receber como processada com erro
 */
const alterarContasReceberFechamentoParaProcessadoErro = async (
  prisma: PrismaClient,
  document: string,
  msg: string,
): Promise<void> => {
  try {
    const sql = `
      EXEC dbo.P_INTEGRACAO_SENIOR_CR_PROCESSADO_ERROR_ALTERAR
        @document = ${toSqlValue(document)},
        @msg = ${toSqlValue(msg)};
    `;
    await prisma.$executeRawUnsafe(sql);
    logger.warn({ document, msg }, 'Conta a receber fechamento marcada como processada com erro');
  } catch (error: any) {
    logger.error(
      { error: error.message, document, msg },
      'Erro ao marcar conta a receber fechamento como processada com erro',
    );
    throw error;
  }
};
...
/**
 * Processa uma conta a receber fechamento
 */
const processarContasReceberFechamento = async (
  prisma: PrismaClient,
  document: string,
): Promise<void> => {
  const processingStartTime = Date.now();
  const eventId = `contas-receber-fechamento-${document}`;

  try {
    logger.info({ document }, 'Iniciando processamento de conta a receber fechamento');
      ...
      webhookEvent = await prisma.webhookEvent.upsert({
        ...
        create: {
          ...
          metadata: JSON.stringify({
            document,
            id: fatura.id,
            etapa: 'processamento_fechamento',
          }),
        },
        update: {
          status: 'processing',
    ...
    logger.info(
      { document },
      'Processando conta a receber fechamento nas tabelas da Senior via stored procedures',
    );
    ...
    if (resultado.success) {
      ...
      await alterarContasReceberFechamentoParaProcessado(prisma, document);
      ...
      logger.info(
        ...
        'Conta a receber fechamento processada com sucesso',
      );
    } else {
      // Marcar como processado com erro
      const msgErro = resultado.error || 'Erro desconhecido';
      await alterarContasReceberFechamentoParaProcessadoErro(prisma, document, msgErro);
      ...
      logger.error(
        ...
        'Falha ao processar conta a receber fechamento',
      );
    }
  } catch (error: any) {
    const processingTimeMs = Date.now() - processingStartTime;
    ...
    logger.error(
      ...
      'Erro ao processar conta a receber fechamento',
    );
  }
};

const processPendingContasReceberBaixaPostgres = async (prisma: PrismaClient): Promise<void> => {
  const candidatos = await prisma.webhookEvent.findMany({
    where: {
      source: '/api/ContasReceber/InserirContasReceberBaixa',
      status: 'pending',
    },
    orderBy: { receivedAt: 'asc' },
    take: CONTAS_RECEBER_BAIXA_BATCH_SIZE,
  });

  if (candidatos.length === 0) {
    logger.debug('Nenhuma baixa de contas a receber pendente para processamento local');
    return;
  }

  logger.info(
    { count: candidatos.length },
    'Processando baixas de contas a receber em modo PostgreSQL local',
  );
  ...
  for (const event of candidatos) {
    ...
    await prisma.webhookEvent.update({
      ...
      data: {
        ...
        metadata: JSON.stringify({
          ...metadataBase,
          ...buildWorkerBypassMetadata('contasReceberBaixaSync'),
          observacao: 'Integracao externa desativada; baixa registrada localmente no PostgreSQL.',
        }).substring(0, 2000),
      },
    });
  }
...
/**
 * Processa contas a receber fechamento pendentes
 */
export async function processPendingContasReceberBaixa(prisma: PrismaClient): Promise<void> {
  if (isPostgresSafeMode()) {
    try {
      await processPendingContasReceberBaixaPostgres(prisma);
    } catch (error: any) {
      logger.error(
        { error: error.message },
        'Erro ao processar baixas de contas a receber em modo PostgreSQL local',
      );
    }
    return;
  }
  ...
  try {
    ...
    if (roboExecutando === 1) {
      logger.debug('Robô CRFECH já está em execução, aguardando...');
      return;
    }
    ...
    try {
      // Listar contas a receber fechamento não integradas
      const documentos = await listarContasReceberFechamentoNaoIntegradas(prisma);

      if (documentos.length === 0) {
        logger.debug('Nenhuma conta a receber fechamento pendente encontrada');
        return;
      }

      logger.info(
        { count: documentos.length },
        'Contas a receber fechamento pendentes encontradas',
      );

      // Processar em lotes
      const batch = documentos.slice(0, CONTAS_RECEBER_BAIXA_BATCH_SIZE);
      for (const document of batch) {
        try {
          await processarContasReceberFechamento(prisma, document);
        } catch (error: any) {
          logger.error(
            { error: error.message, document },
            'Erro ao processar conta a receber fechamento individual',
          );
    ...
    logger.error(
      { error: error.message },
      'Erro ao processar contas a receber fechamento pendentes',
    );
    // Tentar marcar robô como não executando mesmo em caso de erro
    try {
      await alterarExecucaoRobo(prisma, 0);
    } catch (updateError: any) {
      logger.error({ error: updateError.message }, 'Erro ao desmarcar execução do robô CRFECH');
    }
  }
}
